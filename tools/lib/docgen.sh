...
# Get main documentation folder
function main_document_folder()
{
    local top_dir="$1"
...
function theme_download()
{
    ...
    if [ ! -d "${folder}" ] ; then
        ...
        pushd "${folder}" > /dev/null
            cp -R -L sphinx_redactor_theme $THEME_PATH/test_theme || \
...
function static_material_copy()
{
    ...
    local main_docs="$(main_document_folder ${top_dir})"
